...
class SubscriptionClient:
    ...
    def check_income_limit(self, user_id: int, current_count: int) -> bool:
        """Check if user can create more incomes"""
        features = self.get_user_features(user_id)

        if features is None:
            logger.warning(f"Subscription service unreachable, allowing income creation for user {user_id}")
            return True

        income_feature = features.get("incomes", {})

        if not income_feature.get("enabled", False):
            return False
