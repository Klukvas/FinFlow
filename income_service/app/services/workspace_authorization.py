...
from app.exceptions import IncomeValidationError
...
class WorkspaceAuthorizationMixin:
    ...
    def authorize_workspace_access(
        ...
        Raises:
            IncomeValidationError: If user is not authorized
        """
    ...
    def validate_workspace_match(
        self,
        income_workspace_id: UUID,
        requested_workspace_id: UUID,
        income_id: int
    ) -> None:
        """
        Validate that income belongs to the requested workspace.
        
        Args:
            income_workspace_id: Workspace ID from income
            requested_workspace_id: Workspace ID from request
            income_id: Income ID for error message
            
        Raises:
            IncomeValidationError: If workspaces don't match
        """
        if income_workspace_id != requested_workspace_id:
            logger.warning(
                f"Workspace mismatch for income {income_id}",
                category="security",
                operation="workspace_mismatch",
                income_id=income_id,
                income_workspace_id=str(income_workspace_id),
                requested_workspace_id=str(requested_workspace_id)
            )
            raise IncomeValidationError(
                "Income does not belong to the specified workspace"
            )
