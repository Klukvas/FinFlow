...
def upgrade() -> None:
    """
    Backfill workspace_id for all existing incomes.
    
    For each user, get their personal workspace ID and assign it to all their incomes.
    """
    connection = op.get_bind()
    
    # Get all unique user_ids
    result = connection.execute(sa.text("SELECT DISTINCT user_id FROM incomes WHERE workspace_id IS NULL"))
    user_ids = [row[0] for row in result]
    
    print(f"Found {len(user_ids)} users with incomes needing workspace_id")
    
    # For each user, get their default workspace and update their incomes
    for user_id in user_ids:
        workspace_id = get_user_default_workspace(user_id)
        
        if workspace_id:
            print(f"Updating incomes for user {user_id} with workspace {workspace_id}")
            connection.execute(
                sa.text("UPDATE incomes SET workspace_id = :workspace_id WHERE user_id = :user_id AND workspace_id IS NULL"),
...
def downgrade() -> None:
    ...
    connection = op.get_bind()
    connection.execute(sa.text("UPDATE incomes SET workspace_id = NULL"))
