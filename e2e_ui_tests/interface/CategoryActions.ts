...
export class CategoryActions {
  ...
  async createCategory({name, type, parentCategoryName}:CategoryData){
    ...
    await this.expectCreataModalState('hidden')
  }

  async expectCreataModalState(state?: 'hidden' | 'visible'){
  ...
  async expectCategoryCreated(category: CategoryData){
    ...
    await expect(createdCategory.getByTestId('category-type')).toContainText(category.type || 'Расходы')
    if(category.parentCategoryName){
      ...
      await expect(createdCategory.getByTestId('category-parent-name')).toContainText('Корневая категория')
    }
