...
export const Features: React.FC = () => {
 ...
 <span className="inline-block text-xs font-semibold uppercase tracking-widest text-accent-base mb-4">
 {t("featuresPage.hero.label")}
 </span>
 <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-content mb-5 leading-tight">
 ...
 <div className="rounded-2xl border border-[var(--color-border)] p-6 sm:p-10 bg-surface-alt">
 <FeatureGroup
 label={section.label}
 title={section.title}
...
const TechnicalDetails: React.FC<TechnicalDetailsProps> = ({
 ...
 <button
 onClick={onToggle}
 className="flex items-center gap-2 text-xs font-medium text-content-tertiary hover:text-content-secondary transition-colors"
 data-testid={`technical-toggle-${id}`}
 >
 <FaChevronDown
 className={`w-3 h-3 transition-colors ${expanded ? "rotate-180" : ""}`}
 />
 {label}
 </button>
 ...
 <li
 key={i}
 className="text-xs text-content-tertiary flex items-start gap-2"
 >
