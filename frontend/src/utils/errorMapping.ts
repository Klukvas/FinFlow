 ...
 */
export const getCategoryErrorTranslationKey = (
 ...
 */
export const getExpenseErrorTranslationKey = (
  error: BackendError | string,
): string => {
  const errorCode = typeof error === "string" ? null : error.errorCode;
  ...
  const errorMap: Record<string, string> = {
    ...
    EXPENSE_LIMIT_EXCEEDED: "expense.errors.limitExceeded",
  };
 ...
 */
export const getAccountErrorTranslationKey = (
  error: BackendError | string,
): string => {
  const errorCode = typeof error === "string" ? null : error.errorCode;
  ...
  const errorMap: Record<string, string> = {
    ...
    ACCOUNT_LIMIT_EXCEEDED: "account.errors.limitExceeded",
  };
 ...
 */
export const getIncomeErrorTranslationKey = (
  error: BackendError | string,
): string => {
  const errorCode = typeof error === "string" ? null : error.errorCode;
  ...
  const errorMap: Record<string, string> = {
    ...
    INCOME_LIMIT_EXCEEDED: "income.errors.limitExceeded",
  };
 ...
 */
export const getDebtErrorTranslationKey = (
  error: BackendError | string,
): string => {
  const errorCode = typeof error === "string" ? null : error.errorCode;
  ...
  const errorMap: Record<string, string> = {
    ...
    DEBT_LIMIT_EXCEEDED: "debt.errors.limitExceeded",
 ...
 */
export const getGoalsErrorTranslationKey = (
  error: BackendError | string,
): string => {
  const errorCode = typeof error === "string" ? null : error.errorCode;
  ...
  const errorMap: Record<string, string> = {
    ...
    GOAL_LIMIT_EXCEEDED: "goals.errors.limitExceeded",
    ...
    MILESTONE_PROGRESS_UPDATE_FAILED:
      "goals.errors.milestoneProgressUpdateFailed",
  };
 ...
 */
export const getCurrencyErrorTranslationKey = (
  error: BackendError | string,
): string => {
  const errorCode = typeof error === "string" ? null : error.errorCode;
 ...
 */
export const getRecurringErrorTranslationKey = (
  error: BackendError | string,
): string => {
  const errorCode = typeof error === "string" ? null : error.errorCode;
  ...
  const errorMap: Record<string, string> = {
    ...
    RECURRING_LIMIT_EXCEEDED: "recurring.errors.limitExceeded",
  };
...
/**
 * Maps backend error codes to translation keys for workspaces
 */
export const getWorkspaceErrorTranslationKey = (
  error: BackendError | string,
): string => {
  const errorCode = typeof error === "string" ? null : error.errorCode;

  if (!errorCode) {
    return "workspace.errors.genericError";
  }

  const errorMap: Record<string, string> = {
    WORKSPACE_LIMIT_EXCEEDED: "workspace.errors.limitExceeded",
    WORKSPACE_NOT_FOUND: "workspace.errors.notFound",
    WORKSPACE_VALIDATION_ERROR: "workspace.errors.validationError",
  };

  return errorMap[errorCode] || "workspace.errors.genericError";
};
...
export const getErrorTranslationKey = (
  ...
  switch (service) {
    ...
    case "workspace":
      return getWorkspaceErrorTranslationKey(error);
    case "user":
