...
import { AccountResponse, AccountTransactionSummary } from '../../../types/account';
...
export const AccountSidePanel: React.FC<AccountSidePanelProps> = ({
  ...
  const balanceClass = account.balance >= 0
    ? 'text-green-600/80 dark:text-green-400/70 font-semibold'
    : 'text-red-500/80 dark:text-red-400/70 font-semibold';
        ...
        <div className="flex items-center justify-between p-4 sm:p-6 border-b theme-border flex-shrink-0">
          <div className="flex items-center gap-3 min-w-0">
            <div className="min-w-0">
              <h2 className="text-lg font-semibold theme-text-primary truncate">{account.name}</h2>
              <div className="flex items-center gap-2 mt-0.5">
                <Badge variant={getTypeBadgeVariant(account.type) as any} size="sm">
                  {t(ACCOUNT_TYPE_I18N_MAP[account.type] || account.type)}
                </Badge>
                <Badge variant={getStatusBadgeVariant(account.is_archived) as any} size="sm">
                  {account.is_archived ? t('accountPage.table.archived') : t('accountPage.table.active')}
                </Badge>
              </div>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto">
              ...
              <div className="p-4 sm:p-6 space-y-3">
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="text-sm theme-text-secondary">{t('accountPage.sidePanel.income')}</span>
                    <span className="text-sm font-semibold tabular-nums text-green-600/80 dark:text-green-400/70">
                      +{formatBalance(transactionSummary.total_income, account.currency)}
                    </span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-sm theme-text-secondary">{t('accountPage.sidePanel.expenses')}</span>
                    <span className="text-sm font-semibold tabular-nums text-red-500/80 dark:text-red-400/70">
                      -{formatBalance(transactionSummary.total_expenses, account.currency)}
                    </span>
                  </div>
                  <div className="flex items-center justify-between pt-2 border-t theme-border">
                    <span className="text-sm font-medium theme-text-primary">{t('accountPage.sidePanel.netBalance')}</span>
                    <span className={`text-sm font-semibold tabular-nums ${
                      transactionSummary.net_balance >= 0
                        ? 'text-green-600/80 dark:text-green-400/70'
                        : 'text-red-500/80 dark:text-red-400/70'
                    }`}>
                      {formatBalance(transactionSummary.net_balance, account.currency)}
                    </span>
                  </div>
                </div>
              </div>
          ...
          <div className="p-4 sm:p-6 space-y-3">
            <div className="flex items-center justify-between">
              ...
              {transactionSummary && transactionSummary.recent_transactions.length > 0 && (
