...
import { AccountsPageFilters, ACCOUNT_TYPES, ACCOUNT_TYPE_I18N_MAP } from './accountHelpers';
...
export const AccountFilterBar: React.FC<AccountFilterBarProps> = ({
  ...
  const update = (partial: Partial<AccountsPageFilters>) => {
    onFiltersChange({ ...filters, ...partial, page: 1 });
  };

  const clearAll = () => {
    onFiltersChange({ page: 1, size: filters.size, groupBy: filters.groupBy });
  };
    ...
    <div className="theme-surface border theme-border rounded-xl p-4 animate-in fade-in slide-in-from-top-2 duration-200">
      <div className="flex flex-wrap items-end gap-3">
        ...
        <div className="flex flex-col gap-1">
          ...
          >
            <option value="all">{t('accountPage.filters.allCurrencies')}</option>
        ...
        <div className="flex flex-col gap-1">
          ...
          >
            ...
            <option value="currency">{t('accountPage.filters.groupByCurrency')}</option>
