...
export const PeriodSelector: React.FC<PeriodSelectorProps> = ({
    ...
    <div className="flex items-center gap-1">
      {PERIOD_OPTIONS.map((option) => {
        ...
        return (
          <button
            key={option.months}
            data-testid={`period-${option.months}m`}
 ...
 ${
   isActive
     ...
     : isAllowed
       ? "bg-elevated text-content-secondary hover:text-content border-[var(--border)] border"
       : "bg-elevated text-content-tertiary cursor-not-allowed opacity-60 border-[var(--border)] border"
 }
 `}
          >
