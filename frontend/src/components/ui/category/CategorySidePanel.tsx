...
export const CategorySidePanel: React.FC<CategorySidePanelProps> = ({
  ...
  const totalUsage = usageData ? usageData.expenseCount + usageData.incomeCount : 0;
  ...
  const overviewRows = [
    ...
    },
    {
      ...
      value: category.parent_id ? String(category.parent_id) : t('categoryPage.table.rootCategory'),
      valueClass: 'theme-text-primary',
    },
    {
      label: t('categoryPage.sidePanel.source'),
      value: category.created_by === 'SYSTEM'
      ...
      >
        ...
        <div className="flex items-center justify-between p-4 sm:p-6 border-b theme-border flex-shrink-0">
          <div className="flex items-center gap-3 min-w-0">
            <div className="min-w-0">
              <h2 className="text-lg font-semibold theme-text-primary truncate">{category.name}</h2>
              <div className="flex items-center gap-2 mt-0.5">
                ...
                <Badge variant={category.created_by === 'SYSTEM' ? 'secondary' : 'default'} size="sm">
            ...
            {usageLoading ? (
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <span className="text-sm theme-text-secondary">{t('categoryPage.sidePanel.totalTransactions')}</span>
                  <span className="text-sm font-semibold theme-text-primary tabular-nums">{totalUsage}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-sm theme-text-secondary">{t('categoryPage.sidePanel.expenseCount')}</span>
                  <span className="text-sm tabular-nums text-red-500/80 dark:text-red-400/70">{usageData.expenseCount}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-sm theme-text-secondary">{t('categoryPage.sidePanel.incomeCount')}</span>
                  <span className="text-sm tabular-nums text-green-600/80 dark:text-green-400/70">{usageData.incomeCount}</span>
                </div>
                {usageData.totalAmount > 0 && (
                  <>
                    <div className="flex items-center justify-between pt-2 border-t theme-border">
                      <span className="text-sm theme-text-secondary">{t('categoryPage.sidePanel.totalAmount')}</span>
                      <span className="text-sm font-semibold theme-text-primary tabular-nums">
                        {usageData.totalAmount.toLocaleString('uk-UA', { minimumFractionDigits: 2 })} {usageData.currency}
                      </span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-sm theme-text-secondary">{t('categoryPage.sidePanel.averageAmount')}</span>
                      <span className="text-sm theme-text-primary tabular-nums">
                        {usageData.averageAmount.toLocaleString('uk-UA', { minimumFractionDigits: 2 })} {usageData.currency}
                      </span>
                    </div>
                  </>
                )}
              ...
              <div className="p-4 sm:p-6 space-y-3">
                ...
                <div className="space-y-2">
                  {childCategories.map((child) => (
                    ...
                    >
                      <span className="text-sm theme-text-primary">{child.name}</span>
                      <Badge variant={getTypeBadgeVariant(child.type) as any} size="sm">
                    ...
                    {recentTransactions.map((tx) => (
                      <tr key={tx.id}>
                        ...
                        <td className={`px-3 py-2 text-sm font-medium tabular-nums text-right ${
                          ...
                          {tx.amount.toLocaleString('uk-UA', { minimumFractionDigits: 2 })} {tx.currency}
                        </td>
        ...
        <div className="border-t theme-border p-4 sm:p-6 flex-shrink-0">
          <div className="flex flex-wrap gap-2">
            <Button variant="primary" size="sm" onClick={onEdit} className="flex-1 sm:flex-none">
