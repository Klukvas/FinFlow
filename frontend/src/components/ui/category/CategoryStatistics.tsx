...
export const CategoryStatistics: React.FC<CategoryStatisticsProps> = ({
  ...
  if (loading) {
    return (
      <div className="bg-elevated backdrop-blur-sm rounded-xl sm:rounded-2xl border border-[var(--border)] shadow-xl p-4 sm:p-6 lg:p-8">
        ...
        <div className="flex justify-center items-center py-6 sm:py-8">
          <div className="relative">
            <div className="animate-spin rounded-full h-6 w-6 sm:h-8 sm:w-8 border-2 sm:border-3 border-[var(--border)]"></div>
  ...
  if (error) {
    return (
      <div className="bg-elevated backdrop-blur-sm rounded-xl sm:rounded-2xl border border-[var(--border)] shadow-xl p-4 sm:p-6 lg:p-8">
        <div className="flex items-center gap-2 sm:gap-3 mb-4 sm:mb-6">
          <div className="w-6 h-6 sm:w-8 sm:h-8 bg-[var(--danger-dim)] border border-[var(--danger-border)] rounded-lg flex items-center justify-center">
            <svg
              className="w-4 h-4 sm:w-5 sm:h-5 text-[var(--danger)]"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
        ...
        <div className="bg-[var(--danger-dim)] border border-[var(--border)] rounded-lg sm:rounded-xl p-3 sm:p-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-4 lg:gap-6">
        {statItems.map((item, index) => (
          <div
            ...
            className={`relative ${item.bgColor} rounded-lg sm:rounded-xl p-3 sm:p-4 lg:p-6 border border-[var(--border)] hover:shadow-lg transition-all duration-200 hover:-translate-y-1 group`}
          >
            <div className="flex items-center justify-between mb-2 sm:mb-3">
              <div
                className={`w-6 h-6 sm:w-8 sm:h-8 lg:w-10 lg:h-10 bg-[var(--${item.color}-dim)] border border-[var(--${item.color}-border)] rounded-lg flex items-center justify-center group-hover:scale-110 transition-transform duration-200`}
              >
                <svg
                  className={`w-3 h-3 sm:w-4 sm:h-4 lg:w-5 lg:h-5 text-[var(--${item.color})]`}
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
            ...
            <div
              ...
              className="text-xl sm:text-2xl lg:text-3xl xl:text-4xl font-bold text-[var(--text-primary)] mb-1 sm:mb-2"
            >
              {item.value}
            </div>
            <div
