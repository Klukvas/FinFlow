...
import { FaUser, FaCrown, FaEye, FaEdit, FaTrash, FaSpinner, FaChevronDown } from 'react-icons/fa';
      ...
      <div className="flex items-center gap-3 min-w-0 flex-1">
        ...
        <div className={`p-2.5 rounded-full ${
          member.role === 'owner' 
            ...
            : 'bg-gray-100 dark:bg-gray-800 theme-text-secondary'
        }`}>
          {member.role === 'owner' ? (
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="flex items-center gap-2 text-sm theme-text-tertiary mt-0.5">
            ...
            <span>•</span>
            <span>{t('workspace.member.joined', 'Joined')} {formatDate(member.joined_at)}</span>
          </div>
        </div>
      </div>
                ...
                <div className="absolute right-0 top-full mt-1 z-20 w-48 py-1 rounded-lg theme-surface border theme-border shadow-lg">
                  <button
                    ...
                    className={`w-full flex items-center gap-2 px-3 py-2 text-left hover:theme-surface-hover theme-transition ${
                      member.role === 'read' ? 'bg-blue-50 dark:bg-blue-900/20' : ''
                    }`}
                  >
                  ...
                  <button
                    ...
                    className={`w-full flex items-center gap-2 px-3 py-2 text-left hover:theme-surface-hover theme-transition ${
                      member.role === 'full' ? 'bg-emerald-50 dark:bg-emerald-900/20' : ''
                    }`}
                  >
                    <FaEdit className="w-4 h-4 text-emerald-600 dark:text-emerald-400" />
                    <div>
                      <div className="font-medium theme-text-primary">
