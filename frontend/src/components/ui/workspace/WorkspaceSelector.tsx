...
export const WorkspaceSelector: React.FC<WorkspaceSelectorProps> = ({ compact = false }) => {
  ...
  const handleSelect = (workspaceId: string) => {
    console.log('[WorkspaceSelector] Selecting workspace:', workspaceId);
    // First, synchronously set the workspace ID in localStorage
    setStoredWorkspaceId(workspaceId);
    console.log('[WorkspaceSelector] Stored in localStorage, verifying:', localStorage.getItem('current_workspace_id'));
    // Then update React state
    setCurrentWorkspace(workspaceId);
    setIsOpen(false);
    // Small delay to ensure localStorage is committed, then reload
    setTimeout(() => {
      console.log('[WorkspaceSelector] Before reload, localStorage:', localStorage.getItem('current_workspace_id'));
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      >
        ...
        <span className={`theme-text-primary font-medium ${compact ? 'max-w-[100px]' : 'max-w-[150px]'} truncate`}>
        ...
        <div className="absolute top-full left-0 mt-1 w-64 py-2 theme-surface rounded-lg shadow-lg theme-border border z-50">
          ...
          <div className="border-t theme-border mt-2 pt-2">
            ...
            >
              ...
              <span>{t('workspace.manageWorkspaces', 'Manage Workspaces')}</span>
            </Link>
          </div>
        </div>
      )}
