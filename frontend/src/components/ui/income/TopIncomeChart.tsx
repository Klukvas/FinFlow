...
export const TopIncomeChart: React.FC<TopIncomeChartProps> = ({ incomes, categories }) => {
  ...
  return (
    <div className="theme-surface p-4 rounded-lg theme-shadow theme-border border">
      <h2 className="text-lg font-semibold mb-4 theme-text-primary">{t('incomePage.dashboard.topTitle')}</h2>
      <div className="h-[300px]">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={topData} layout="vertical">
            <CartesianGrid strokeDasharray="3 3" stroke="var(--color-border)" strokeOpacity={0.5} />
            <XAxis type="number" tick={{ fill: 'var(--color-text-primary)', fontSize: 12 }} axisLine={{ stroke: 'var(--color-border)' }} />
            <YAxis dataKey="name" type="category" width={100} tick={{ fill: 'var(--color-text-primary)', fontSize: 12 }} axisLine={{ stroke: 'var(--color-border)' }} />
