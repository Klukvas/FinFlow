  ...
  return (
    ...
    >
      ...
      <div>
        <label
          htmlFor="title"
          className="block text-sm font-medium text-content mb-1 sm:mb-2"
        >
        ...
        />
        {errors.title && (
          <p className="mt-1 text-sm text-danger-base">{errors.title}</p>
        )}
      </div>
      ...
      <div>
        ...
        <textarea
          ...
          rows={3}
          className="w-full px-3 py-3 sm:py-2 text-base sm:text-sm border-[var(--border)] border rounded-md focus:outline-none focus:ring-2 focus:ring-accent transition-colors touch-manipulation bg-surface text-content"
            ...
            {GOAL_PRIORITIES.map((priority) => {
              ...
              return (
                <option key={priority} value={priority}>
                  {t(`goalsPage.filters.${priorityKey}`)}
                </option>
              );
            })}
          </select>
      ...
      <div>
        ...
        />
        {errors.target_date && (
          <p className="mt-1 text-sm text-danger-base">{errors.target_date}</p>
        )}
      </div>
      ...
      <div className="flex items-start sm:items-center gap-3">
        <input
          ...
          onChange={(e) =>
            handleInputChange("is_milestone_based", e.target.checked)
          }
          className="h-5 w-5 mt-0.5 sm:mt-0 text-accent-base focus:ring-accent border-[var(--border)] border rounded touch-manipulation"
        />
        <label
          htmlFor="is_milestone_based"
          className="block text-sm text-content leading-relaxed"
        >
      ...
      <div className="flex flex-col sm:flex-row justify-end gap-3 pt-2">
        ...
        >
          {isLoading
            ? t("goalsPage.form.saving")
            : goal
              ? t("goalsPage.form.update")
              : t("goalsPage.form.create")}
        </Button>
      </div>
    </form>
  );
