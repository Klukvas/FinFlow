...
export const GoalTable: React.FC<GoalTableProps> = ({
  ...
  const SortIcon = ({ field }: { field: SortField }) => (
    <svg className={`w-3 h-3 ml-1 inline-block ${sortField === field ? 'theme-text-primary' : 'theme-text-tertiary'}`} viewBox="0 0 12 12" fill="currentColor">
  ...
  return (
    <div>
      ...
      <div className="px-4 sm:px-6 py-3 border-b theme-border flex items-center gap-4 text-xs font-medium theme-text-secondary">
        <button onClick={() => toggleSort('name')} className="hover:theme-text-primary transition-colors">
          {t('goalsPage.table.name')} <SortIcon field="name" />
        </button>
        <button onClick={() => toggleSort('progress')} className="hover:theme-text-primary transition-colors">
          {t('goalsPage.table.progress')} <SortIcon field="progress" />
        </button>
        <button onClick={() => toggleSort('targetDate')} className="hover:theme-text-primary transition-colors">
          ...
          return (
            ...
            >
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-x-4 gap-y-1.5 text-xs">
                <div>
                  <span className="theme-text-tertiary">{t('goalsPage.table.info.currentAmount')}</span>
                  <span className="ml-1 font-medium text-green-600/80 dark:text-green-400/70 tabular-nums">
                    {formatAmount(goal.current_amount, goal.currency)}
                  </span>
                </div>
                <div>
                  <span className="theme-text-tertiary">{t('goalsPage.table.info.targetAmount')}</span>
                  <span className="ml-1 font-medium theme-text-primary tabular-nums">
                    {formatAmount(goal.target_amount, goal.currency)}
                  </span>
                </div>
                <div>
                  <span className="theme-text-tertiary">{t('goalsPage.table.info.requiredMonthly')}</span>
                  <span className="ml-1 font-medium theme-text-primary tabular-nums">
                    {requiredMonthly !== null ? formatAmount(requiredMonthly, goal.currency) : '\u2014'}
                  </span>
                </div>
                <div>
                  <span className="theme-text-tertiary">{t('goalsPage.table.info.targetDate')}</span>
                  <span className="ml-1 font-medium theme-text-primary">
                    {formatDate(goal.target_date)}
                  </span>
                </div>
                <div>
                  <span className="theme-text-tertiary">{t('goalsPage.table.info.daysRemaining')}</span>
                  <span className={`ml-1 font-medium tabular-nums ${isOverdue ? 'text-red-500/80 dark:text-red-400/70' : 'theme-text-primary'}`}>
                    {daysRemaining !== null
                      ? (isOverdue
                ...
                <div className="flex items-center gap-1.5">
                  <span className={`w-2 h-2 rounded-full flex-shrink-0 ${getTrackDotColor(trackStatus)}`} />
                  <span className={`font-medium ${getTrackStatusColor(trackStatus)}`}>
