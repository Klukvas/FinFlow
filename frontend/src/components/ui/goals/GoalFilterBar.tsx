...
export const GoalFilterBar: React.FC<GoalFilterBarProps> = ({
  ...
  const update = (partial: {
    [K in keyof GoalsPageFilters]?: GoalsPageFilters[K] | undefined;
  }) => {
    const merged = { ...filters, ...partial, page: 1 };
    (Object.keys(merged) as (keyof typeof merged)[]).forEach((k) => {
      if (merged[k] === undefined) delete merged[k];
    });
    onFiltersChange(merged as GoalsPageFilters);
  };

  const clearAll = () => {
    const base: GoalsPageFilters = { page: 1, size: filters.size };
