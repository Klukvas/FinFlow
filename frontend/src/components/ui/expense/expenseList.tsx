        ...
        const sorted = [...expenses].sort((a, b) => {
            ...
            switch (sortField) {
                ...
                case 'amount':
                    ...
                    break;
                case 'category':
                    cmp = (categories[a.category_id] || '').localeCompare(categories[b.category_id] || '');
                    break;
            }
    ...
    if (loading) {
        return (
            <div className="flex justify-center items-center py-12">
                ...
                <span className="ml-3 theme-text-secondary text-sm">{t('expense.list.loading')}</span>
            </div>
        );
    }
        ...
        return (
            <div className="text-center py-12">
                ...
                <h3 className="text-base font-semibold theme-text-primary mb-1">
                    {emptyMessage || (totalItems === 0 ? t('expense.list.noExpensesAtAll') : t('expense.list.noExpensesOnPage'))}
                </h3>
                <p className="theme-text-secondary text-sm max-w-md mx-auto">
                    {totalItems === 0 ? t('expense.list.noExpensesSubtitle') : t('expense.list.noExpensesOnPageSubtitle')}
                </p>
            </div>
        );
    }
                        ...
                        <div className="flex items-start justify-between">
                            <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-2 mb-1">
                                    <span className="text-sm font-semibold text-red-500/80 dark:text-red-400/70">
                    ...
                    <table className="w-full">
                        <thead className="theme-bg-secondary sticky top-0 z-10">
                            <tr>
                                ...
                                >
                                    {t('expense.list.date')}<SortIcon field="date" />
                                </th>
                                <th
                                ...
                                >
                                    {t('expense.list.amount')}<SortIcon field="amount" />
                                </th>
                                ...
                                >
                                    {t('expense.list.category')}<SortIcon field="category" />
                                </th>
                                <th className="px-4 py-3 text-left text-xs font-semibold theme-text-secondary uppercase tracking-wider">
                        ...
                        <tbody className="theme-surface divide-y theme-border">
                            {sortedExpenses.map((expense) => (
                                <tr key={expense.id} className="hover:theme-surface-hover transition-colors group">
