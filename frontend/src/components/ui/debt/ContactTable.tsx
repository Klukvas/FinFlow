  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center py-12">
        ...
        <span className="ml-3 theme-text-secondary text-sm">{t('debtPage.contactsTable.loading')}</span>
      </div>
    );
  }
          ...
          >
            <div className="flex items-start justify-between">
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium theme-text-primary">{contact.name}</p>
                {contact.company && (
                  <p className="text-xs theme-text-tertiary mt-0.5">{contact.company}</p>
                )}
                {contact.email && (
                  <a href={`mailto:${contact.email}`} className="text-xs theme-accent hover:underline mt-0.5 block">
                    {contact.email}
                  </a>
                )}
                <div className="flex items-center gap-3 mt-1.5">
                  <span className="text-xs theme-text-secondary">
                    {contact.debts_count} {t('debtPage.contactsTable.debtCount').toLowerCase()}
                  </span>
                  {contact.total_debt_amount > 0 && (
                    <span className="text-xs text-red-500/80 dark:text-red-400/70 tabular-nums">
        ...
        <div className="overflow-hidden">
          <table className="w-full">
            <thead className="theme-bg-secondary sticky top-0 z-10">
              <tr>
                <th className={`${thClasses} text-left`}>{t('debtPage.contactsTable.name')}</th>
                <th className={`${thClasses} text-left`}>{t('debtPage.contactsTable.company')}</th>
                <th className={`${thClasses} text-left`}>{t('debtPage.contactsTable.email')}</th>
                <th className={`${thClasses} text-right`}>{t('debtPage.contactsTable.debtCount')}</th>
                <th className={`${thClasses} text-right`}>{t('debtPage.contactsTable.totalDebt')}</th>
                <th className={`${thClasses} text-right`}>{t('debtPage.contactsTable.actions')}</th>
              </tr>
            </thead>
            <tbody className="theme-surface divide-y theme-border">
              {enrichedContacts.map((contact) => (
                <tr key={contact.id} className="hover:theme-surface-hover transition-colors group">
                  <td className="px-4 py-3">
                    <span className="text-sm font-medium theme-text-primary">{contact.name}</span>
                  </td>
                  <td className="px-4 py-3">
                    <span className="text-sm theme-text-secondary">{contact.company || '—'}</span>
                  </td>
                  <td className="px-4 py-3">
                    {contact.email ? (
                      <a href={`mailto:${contact.email}`} className="text-sm theme-accent hover:underline">
                        {contact.email}
                      </a>
                    ) : (
                  ...
                  <td className="px-4 py-3 text-right">
                    <span className="text-sm theme-text-secondary tabular-nums">{contact.debts_count}</span>
                  </td>
                  <td className="px-4 py-3 text-right">
                    <span className="text-sm text-red-500/80 dark:text-red-400/70 tabular-nums">
                      {contact.total_debt_amount > 0
                        ? formatCurrency(contact.total_debt_amount, userCurrency)
                        : '—'}
                    </span>
