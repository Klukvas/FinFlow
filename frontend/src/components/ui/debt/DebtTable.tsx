  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center py-12">
        ...
        <span className="ml-3 theme-text-secondary text-sm">{t('debtPage.table.loading')}</span>
      </div>
    );
  }
    ...
    return (
      <div className="text-center py-12">
        ...
        <h3 className="text-base font-semibold theme-text-primary mb-1">
          {emptyMessage || (totalItems === 0 ? t('debtPage.table.noDebtsAtAll') : t('debtPage.table.noDebtsOnPage'))}
        </h3>
        <p className="theme-text-secondary text-sm max-w-md mx-auto">
          {totalItems === 0 ? t('debtPage.table.noDebtsSubtitle') : t('debtPage.table.noDebtsOnPageSubtitle')}
        </p>
      </div>
    );
  }
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-3 mb-2">
                    <span className="text-sm font-semibold text-red-500/80 dark:text-red-400/70 tabular-nums">
                      {Math.abs(debt.current_balance).toLocaleString('uk-UA', { minimumFractionDigits: 2 })} {debt.currency}
                    </span>
                    {debt.due_date && (
                      <span className="text-xs theme-text-tertiary">{formatDate(debt.due_date)}</span>
                    )}
                  </div>
                  {/* Progress bar */}
                ...
                <div className="flex items-center gap-1 ml-2 flex-shrink-0">
                  {!debt.is_paid_off && (
                    <Button
                      variant="ghost"
                      size="sm"
                      onClick={(e) => { e.stopPropagation(); onMakePayment(debt); }}
            ...
            <thead className="theme-bg-secondary sticky top-0 z-10">
              <tr>
                <th className={`${sortableTh} text-left`} onClick={() => toggleSort('name')}>
                  {t('debtPage.table.name')}<SortIcon field="name" />
                </th>
                <th className={`${sortableTh} text-right`} onClick={() => toggleSort('balance')}>
                  {t('debtPage.table.balance')}<SortIcon field="balance" />
                </th>
                <th className={`${sortableTh} text-right`} onClick={() => toggleSort('initial')}>
                  {t('debtPage.table.initialAmount')}<SortIcon field="initial" />
                </th>
                <th className={`${sortableTh} text-right`} onClick={() => toggleSort('interest')}>
                  {t('debtPage.table.interest')}<SortIcon field="interest" />
                </th>
                <th className={`${sortableTh} text-left`} onClick={() => toggleSort('dueDate')}>
                  {t('debtPage.table.dueDate')}<SortIcon field="dueDate" />
                </th>
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-right">
                      <span className="text-sm font-semibold text-red-500/80 dark:text-red-400/70 tabular-nums">
                        {Math.abs(debt.current_balance).toLocaleString('uk-UA', { minimumFractionDigits: 2 })}
                      </span>
                      <span className="text-xs theme-text-tertiary ml-1">{debt.currency}</span>
                    </td>
                    <td className="px-4 py-3 text-right">
                      <span className="text-sm theme-text-secondary tabular-nums">
                        {Math.abs(debt.initial_amount).toLocaleString('uk-UA', { minimumFractionDigits: 2 })}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-right">
