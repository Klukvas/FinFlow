...
import { RecurringPayment, PaymentSchedule } from '../../../services/api/recurringApi';
...
export const RecurringSidePanel: React.FC<RecurringSidePanelProps> = ({
  ...
  const isExpense = payment.payment_type === 'expense' || payment.payment_type === ('EXPENSE' as string);
        ...
        <div className="flex items-center justify-between p-4 sm:p-6 border-b theme-border flex-shrink-0">
          <div className="flex items-center gap-3 min-w-0">
            <div className="min-w-0">
              <h2 className="text-lg font-semibold theme-text-primary truncate">{payment.name}</h2>
              <div className="flex items-center gap-2 mt-0.5">
                <span className="text-xs theme-text-tertiary">
                  {t(SCHEDULE_TYPE_I18N_MAP[payment.schedule_type])}
                </span>
                <Badge
                  variant={getStatusBadgeVariant(payment.status) as 'success' | 'warning' | 'info' | 'destructive'}
                  size="sm"
                >
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <div className="p-4 sm:p-6 space-y-3">
            <div className="flex items-center justify-between">
              ...
              {schedules.length > 0 && (
                <span className="text-xs theme-text-tertiary">
                  {t('recurringPage.sidePanel.executionsCount', { count: schedules.length })}
                </span>
              )}
            </div>
                    ...
                    {schedules.map((schedule) => (
                      <tr key={schedule.id}>
                        ...
                        <td className="px-3 py-2">
                          <Badge
                            variant={getScheduleStatusBadgeVariant(schedule.status) as 'success' | 'destructive' | 'secondary'}
                            size="sm"
                          >
                            {t(`recurringPage.sidePanel.scheduleStatus.${schedule.status}`)}
                          </Badge>
                        </td>
                        <td className="px-3 py-2 text-sm text-red-500/80 dark:text-red-400/70">
          ...
          <div className="p-4 sm:p-6 space-y-2">
            ...
            <p className={`text-sm ${payment.description ? 'theme-text-secondary' : 'theme-text-tertiary italic'}`}>
