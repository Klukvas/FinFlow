...
export const RecurringFilterBar: React.FC<RecurringFilterBarProps> = ({
 ...
 [K in keyof RecurringFilters]?: RecurringFilters[K] | undefined;
 }) => {
 const merged = { ...filters, ...partial, page: 1 };
 (Object.keys(merged) as (keyof typeof merged)[]).forEach((k) => {
 if (merged[k] === undefined) delete merged[k];
 });
 onFiltersChange(merged as RecurringFilters);
 };

 const clearAll = () => {
 const base: RecurringFilters = { page: 1, size: filters.size };
 ...
 >
 <option value="all">
 {t("recurringPage.filters.allFrequencies")}
 </option>
