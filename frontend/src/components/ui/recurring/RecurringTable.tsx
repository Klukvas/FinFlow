...
type SortField = 'name' | 'amount' | 'schedule' | 'nextExecution' | 'lastExecuted' | 'status';
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center py-12">
        ...
        <span className="ml-3 theme-text-secondary text-sm">{t('recurringPage.table.loading')}</span>
      </div>
    );
  }
    ...
    return (
      <div className="text-center py-12">
        ...
        <h3 className="text-base font-semibold theme-text-primary mb-1">
          {emptyMessage || (totalItems === 0 ? t('recurringPage.table.noPaymentsAtAll') : t('recurringPage.table.noPaymentsOnPage'))}
        </h3>
        <p className="theme-text-secondary text-sm max-w-md mx-auto">
          {totalItems === 0 ? t('recurringPage.table.noPaymentsSubtitle') : t('recurringPage.table.noPaymentsOnPageSubtitle')}
        </p>
      </div>
    );
  }
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <Badge variant={getStatusBadgeVariant(status) as 'success' | 'warning' | 'info' | 'destructive'} size="sm">
                      {t(STATUS_I18N_MAP[status])}
                    </Badge>
                  </div>
                  <div className="text-xs theme-text-tertiary mb-1">
                    {t(PAYMENT_TYPE_I18N_MAP[payment.payment_type])} · {getScheduleDescription(payment, t)}
                  </div>
                ...
                <div className="flex items-center gap-1 ml-2 flex-shrink-0">
                  {(payment.status === 'active' || payment.status === 'paused') && (
                    <Button
                      variant="ghost"
                      size="sm"
                      onClick={(e) => { e.stopPropagation(); onPauseResume(payment); }}
            ...
            <thead className="theme-bg-secondary sticky top-0 z-10">
              <tr>
                <th className={`${sortableTh} text-left`} onClick={() => toggleSort('name')}>
                  {t('recurringPage.table.name')}<SortIcon field="name" />
                </th>
                <th className={`${sortableTh} text-right`} onClick={() => toggleSort('amount')}>
                  {t('recurringPage.table.amount')}<SortIcon field="amount" />
                </th>
                <th className={`${sortableTh} text-left`} onClick={() => toggleSort('schedule')}>
                  {t('recurringPage.table.frequency')}<SortIcon field="schedule" />
                </th>
                <th className={`${sortableTh} text-left`} onClick={() => toggleSort('nextExecution')}>
                  {t('recurringPage.table.nextExecution')}<SortIcon field="nextExecution" />
                </th>
                <th className={`${sortableTh} text-left`} onClick={() => toggleSort('lastExecuted')}>
                  {t('recurringPage.table.lastExecuted')}<SortIcon field="lastExecuted" />
                </th>
                <th className={`${sortableTh} text-left`} onClick={() => toggleSort('status')}>
                  {t('recurringPage.table.status')}<SortIcon field="status" />
                </th>
                ...
                return (
                  ...
                  >
                    <td className="px-4 py-3">
                      <div>
                        <span className="text-sm font-medium theme-text-primary">{payment.name}</span>
                      ...
                      <div className="flex items-center justify-end gap-1">
                        {(payment.status === 'active' || payment.status === 'paused') && (
                          <Button
                            ...
                            title={payment.status === 'active' ? t('recurringPage.sidePanel.pause') : t('recurringPage.sidePanel.resume')}
                          >
                            {payment.status === 'active' ? (
                              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 9v6m4-6v6m7-3a9 9 0 11-18 0 9 9 0 0118 0z" />
