...
import { Lock, Play, Pause, Pencil } from "lucide-react";
...
export const RecurringPaymentCard: React.FC<RecurringPaymentCardProps> = ({
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case "active":
        return "bg-[var(--success-dim)] text-success-base";
      case "paused":
        return "bg-[var(--warning-dim)] text-warning-base";
      case "completed":
        return "bg-[var(--accent-dim)] text-accent-base";
      case "cancelled":
        return "bg-[var(--danger-dim)] text-danger-base";
      default:
  ...
  return (
    <div className="bg-elevated rounded-lg theme-shadow p-6 border-[var(--border)] border">
      ...
      <div className="grid grid-cols-2 gap-4 mb-4">
        <div>
          ...
          <p className="text-lg font-mono font-semibold text-content">
      ...
      <div className="flex justify-end space-x-2">
        {payment.status === "active" && (
          <button
            ...
            className="p-2 text-warning-base hover:bg-[var(--warning-dim)] rounded-md transition-colors"
            title="Приостановить"
          >
        ...
        {payment.status === "paused" && (
          <button
            ...
            className="p-2 text-success-base hover:bg-[var(--success-dim)] rounded-md transition-colors"
            title="Возобновить"
          >
        ...
        <button
          ...
          className={`p-2 rounded-md transition-colors ${
            payment.is_read_only
              ...
              : "text-accent-base hover:bg-[var(--accent-dim)]"
          }`}
          title={
            payment.is_read_only
