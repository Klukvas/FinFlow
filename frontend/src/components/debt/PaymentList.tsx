    ...
    return (
      <div className="space-y-4">
        {[...Array(3)].map((_, index) => (
          <Card
            key={index}
            className="animate-pulse bg-elevated border-[var(--border)] border"
          >
    ...
    return (
      <Card className="bg-elevated border-[var(--border)] border">
        <CardContent className="p-8 text-center">
          <DollarSign className="w-12 h-12 mx-auto mb-4 text-content-tertiary" />
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card className="bg-elevated border-[var(--border)] border">
          <CardContent className="p-4">
            <div className="flex items-center space-x-2 mb-2">
              <TrendingDown className="w-5 h-5 text-success-base" />
        ...
        <Card className="bg-elevated border-[var(--border)] border">
          <CardContent className="p-4">
            <div className="flex items-center space-x-2 mb-2">
              <DollarSign className="w-5 h-5 text-accent-base" />
              <span className="text-sm font-medium text-content-secondary">
                Principal Paid
              </span>
            </div>
        ...
        <Card className="bg-elevated border-[var(--border)] border">
          <CardContent className="p-4">
            <div className="flex items-center space-x-2 mb-2">
              <TrendingUp className="w-5 h-5 text-danger-base" />
              <span className="text-sm font-medium text-content-secondary">
                Interest Paid
              </span>
            </div>
      ...
      {/* Payment List */}
      <div className="space-y-3">
        <h3 className="text-lg font-semibold text-content">Payment History</h3>

        {payments.map((payment) => (
          <Card
            key={payment.id}
            className="transition-all duration-200 hover:shadow-md bg-elevated border-[var(--border)] border hover:border-[var(--border)]"
          >
            <CardContent className="p-4">
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center space-x-3 mb-2">
                    <div className="p-2 rounded-lg bg-[var(--success-dim)]">
                      <DollarSign className="w-4 h-4 text-success-base" />
                    </div>
                    <div>
                      <h4 className="font-mono font-semibold text-content">
                  ...
                  {(payment.principal_amount || payment.interest_amount) && (
                    <div className="flex space-x-4 mb-2">
                      ...
                      {payment.interest_amount && (
                        <div className="flex items-center space-x-1">
                          <div className="w-2 h-2 rounded-full bg-danger-base" />
                          <span className="text-xs text-content-tertiary">
                            Interest:{" "}
