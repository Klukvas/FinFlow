...
const CANCELLATION_REASONS = [
  ...
  { value: 'switching_service', label: 'cancellation.reasons.switchingService' },
...
export const CancelSubscriptionModal: React.FC<CancelSubscriptionModalProps> = ({
  ...
  const formattedExpiryDate = expiresAt ? expiresAt.toLocaleDateString() : 'N/A';
  ...
  const handleCancel = async () => {
    ...
    try {
      ...
      toast.success(
        cancelImmediately
          ? t('cancellation.successImmediate', 'Subscription canceled immediately')
      ...
      <div className="space-y-6">
        ...
        <div className="theme-bg-secondary p-4 rounded-lg border theme-border">
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
            ...
            <div>
              <p className="theme-text-secondary">{t('subscription.periodEnds', 'Period Ends')}:</p>
              <p className="font-medium theme-text-primary">{formattedExpiryDate}</p>
            </div>
            {subscription.amount && (
              <div>
                <p className="theme-text-secondary">{t('subscription.amount', 'Amount')}:</p>
                <p className="font-medium theme-text-primary">
        ...
        <div className="space-y-4">
          ...
          <label className="flex items-start gap-3 p-4 rounded-lg border theme-border cursor-pointer hover:theme-bg-secondary transition-colors">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm theme-text-secondary">
                {t('cancellation.immediatelyDesc', 'Lose access now. May issue pro-rata refund.')}
              </p>
            </div>
          </label>
        </div>
        ...
        <div className="space-y-2">
          <label htmlFor="cancelReason" className="text-sm font-medium theme-text-primary">
            {t('cancellation.reason', 'Reason for canceling')} ({t('common.optional', 'optional')})
          </label>
          <select
            id="cancelReason"
            value={reason}
          ...
          >
            <option value="">{t('cancellation.selectReason', 'Select a reason...')}</option>
        ...
        {/* Comment (Optional) */}
        <div className="space-y-2">
          <label htmlFor="cancelComment" className="text-sm font-medium theme-text-primary">
          ...
          <textarea
            ...
            rows={3}
            maxLength={500}
            placeholder={t('cancellation.commentPlaceholder', 'Tell us why you\'re canceling...')}
        ...
        <div className="theme-bg-secondary p-4 rounded-lg border theme-border">
          ...
          <ul className="space-y-1 text-sm theme-text-secondary">
            <li className="flex items-start gap-2">
              ...
              <span>{t('cancellation.noFutureCharges', 'No future charges will be made')}</span>
            </li>
            {!cancelImmediately && (
              <li className="flex items-start gap-2">
                <span className="theme-success mt-1">✓</span>
            ...
            <li className="flex items-start gap-2">
              <span className="theme-success mt-1">✓</span>
              <span>{t('cancellation.dataPreserved', 'Your data will be preserved')}</span>
            </li>
            <li className="flex items-start gap-2">
              <span className="theme-success mt-1">✓</span>
              <span>{t('cancellation.canReactivate', 'You can reactivate anytime')}</span>
            </li>
          </ul>
        </div>
