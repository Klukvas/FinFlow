...
export class SubscriptionApiClient {
  ...
  async getUserEntitlements(
    userId: number,
  ): Promise<UserEntitlements | ApiError> {
  ...
  async getUserFeatures(userId: number): Promise<UserFeature[] | ApiError> {
    ...
    const features: UserFeature[] = Object.entries(
      entitlements.entitlements,
    ).map(([feature_code, data]) => ({
      feature_code,
  ...
  ): Promise<{ message: string; effective_date?: string } | ApiError> {
    return this.httpClient.deleteWithBody<{
      message: string;
      effective_date?: string;
    }>(`/v1/subscriptions/${userId}/cancel`, data);
  }
  ...
  async getCurrentCounts(
    ...
    try {
      ...
      const [accounts, categories, expenses, incomes, debts, goals] =
        await Promise.allSettled([
          this.accountApi.getAccounts(), // This endpoint doesn't support pagination yet, so we fetch all
          this.categoryApi.getCategoriesPaginated({
            page: 1,
            size: 1,
            flat: true,
          }),
      ...
      if (accounts.status === "fulfilled" && !("error" in accounts.value)) {
        counts.accounts = Array.isArray(accounts.value)
      ...
      if (categories.status === "fulfilled" && !("error" in categories.value)) {
        ...
        counts.categories =
          "total" in categories.value ? categories.value.total : 0;
      }
