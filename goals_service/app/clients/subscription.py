...
class SubscriptionClient:
    ...
    def check_goal_limit(self, user_id: int, current_count: int) -> bool:
        """Check if user can create more goals"""
        features = self.get_user_features(user_id)

        if features is None:
            logger.warning(f"Subscription service unreachable, allowing goal creation for user {user_id}")
            return True

        goal_feature = features.get("goals", {})

        if not goal_feature.get("enabled", False):
            return False

        limit = goal_feature.get("limit_value")
        if limit is None:
            return True
