...
class TestCategorySerializer:
    ...
    def test_validate_uniqueness_with_exclude(self):
        """Test uniqueness validation with exclude_id (no conflict found)."""
        # When exclude_id is provided the code chains two .filter() calls.
        # Configure the mock so any chained .filter() also returns no result.
    ...
    def test_serialize_category_for_update_partial(self):
        """Test partial serialization for category update (name only)."""
        # CategoryCreate.type has a default of EXPENSE and cannot be None.
        # When only name is provided, type defaults to EXPENSE and parent_id defaults to None.
        data = CategoryCreate(
            name="Updated Category",
            parent_id=None,
        )
        ...
        # parent_id=None is excluded by serialize_category_for_update (checks `is not None`).
        # type defaults to EXPENSE and is also excluded if checked `is not None` — but EXPENSE is not None.
        # Looking at the implementation: only includes type if `data.type is not None`.
        # CategoryType.EXPENSE is not None, so it IS included.
    ...
    def test_validate_category_data_with_exclude(self):
        ...
        with patch.object(self.serializer, 'validate_category_type') as mock_type, \
            ...
            mock_type.assert_called_once_with(CategoryType.EXPENSE)
            # validate_uniqueness is called with workspace_id=None as 5th argument
            mock_unique.assert_called_once_with(
                self.db, self.user_id, {"name": "Test Category"}, exclude_id, None
            )
            mock_parent.assert_called_once_with(self.db, self.parent_id, self.user_id)
