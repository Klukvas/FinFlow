...
class SubscriptionClient:
    ...
    def check_debt_limit(self, user_id: int, current_count: int) -> bool:
        """Check if user can create more debts"""
        features = self.get_user_features(user_id)

        if features is None:
            logger.warning(f"Subscription service unreachable, allowing debt creation for user {user_id}")
            return True

        debt_feature = features.get("debts", {})

        if not debt_feature.get("enabled", False):
            return False

        limit = debt_feature.get("limit_value")
        if limit is None:
            return True
