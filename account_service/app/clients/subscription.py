...
class SubscriptionClient:
    ...
    def check_account_limit(self, user_id: int, current_count: int) -> bool:
        """Check if user can create more accounts"""
        features = self.get_user_features(user_id)

        if features is None:
            logger.warning(f"Subscription service unreachable, allowing account creation for user {user_id}")
            return True

        account_feature = features.get("accounts", {})

        if not account_feature.get("enabled", False):
            return False

        limit = account_feature.get("limit_value")
        if limit is None:
            return True
