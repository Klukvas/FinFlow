...
class SubscriptionClient:
    ...
    def check_expense_limit(self, user_id: int, current_count: int) -> bool:
        """Check if user can create more expenses"""
        features = self.get_user_features(user_id)

        if features is None:
            logger.warning(f"Subscription service unreachable, allowing expense creation for user {user_id}")
            return True

        expense_feature = features.get("expenses", {})

        if not expense_feature.get("enabled", False):
            return False
