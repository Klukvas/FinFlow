...
def upgrade() -> None:
    ...
    op.execute("""
        ...
        SELECT p.id, f.code, true, 
            CASE f.code
                WHEN 'categories' THEN 10          -- 10 categories max
                WHEN 'accounts' THEN 2             -- 2 accounts max
                WHEN 'expenses' THEN 100           -- 100 expenses per month
                WHEN 'incomes' THEN 100            -- 100 incomes per month
                WHEN 'debts' THEN 2                -- 2 debts max
                WHEN 'recurring' THEN 3            -- 3 recurring transactions
                WHEN 'goals' THEN 2                -- 2 goals max
    ...
    op.execute("""
        ...
        SELECT p.id, f.code, true, 
            CASE f.code
                WHEN 'categories' THEN 50          -- 50 categories max
                WHEN 'accounts' THEN 10            -- 10 accounts max
                WHEN 'expenses' THEN 5000          -- 5000 expenses per month
                WHEN 'incomes' THEN 5000           -- 5000 incomes per month
                WHEN 'debts' THEN 20               -- 20 debts max
                WHEN 'recurring' THEN 50           -- 50 recurring transactions
                WHEN 'goals' THEN 20               -- 20 goals max
                WHEN 'workspaces' THEN 3           -- 3 workspaces
            END
        FROM plans p
        CROSS JOIN features f
    ...
    op.execute("""
        ...
        SELECT p.id, f.code, true, 
            CASE f.code
                ...
                WHEN 'expenses' THEN NULL          -- Unlimited expenses
                WHEN 'incomes' THEN NULL           -- Unlimited incomes
                ...
                WHEN 'workspaces' THEN 10          -- 10 workspaces
            END
        FROM plans p
        CROSS JOIN features f
